...
def main():
  ...
  html2="""
    <div style="text-align:center; text-shadow: 3px 1px 2px purple;">
      <h2><b>Your Friendly Career Advisor<h2></b>
    </div>
      """
...
# Check if all fields are filled
  if Name and Contact_Number and Email_address:
    if submit_button_clicked:
        # Store the user's information in an Excel sheet
        data = {'Full Name': [Name], 'Contact Number': [Contact_Number], 'Email address': [Email_address]}
        new_df = pd.DataFrame(data)

        # Specify the full path to the Excel file
        file_path = os.path.abspath("D:\\Career-Prediction-System-main\\user_info.xlsx")
        ...
        st.sidebar.success("Thanks for submitting!")
  else:
    if submit_button_clicked:
